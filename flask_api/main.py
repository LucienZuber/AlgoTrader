...
def open_position(pair: str, order_type: str, size: float, tp_distance: int = None, stop_distance: int = None, comment: str = ""):
...
def close_position(deal_id, comment: str = "Close trade"):
...
def get_data(time_frame: int):
    ...
    pair_data = dict()
    for pair in pairs:
        utc_from = datetime(2021, 1, 1, tzinfo=pytz.timezone('Europe/Berlin'))
...
def check_timedout_positions(data: pd.DataFrame):
...
def check_trades(time_frame: int, pair_data: dict):
...
def run_trader(time_frame: int):
    ...
    pair_data = get_data(time_frame=time_frame)
